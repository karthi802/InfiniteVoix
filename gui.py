...
class ChatInterface(Frame):
    ...
    def __init__(self, master=None):
        ...
        self.mic_button = Button(self.send_button_frame, text="mic", width=5, relief=GROOVE, bg='white',
                                  bd=1, command=lambda: self.mic_message_insert(), activebackground="#FFFFFF",
                                  activeforeground="#000000")
        self.mic_button.pack(side=LEFT, ipady=7)
        self.master.bind("<Return>", self.mic_message_insert)
    ...
    def msg(self):
        tkinter.messagebox.showinfo("InfiniteVoix v1.0",
                                    'InfiniteVoix is a chatbot for answering questions\nIt is based on NLP using pythons NLTK tool-kit module\nGUI is based on Tkinter')

    def about(self):
        tkinter.messagebox.showinfo("InfiniteVoix Developers","Karthikeyan and Kevin samson")
    ...
    def speech_to_text(self):
        mic_input = sr.takeCommand()
        return mic_input
        # pr1 = "Human : " + self.user_input + "\n"
        # self.text_box.configure(state=NORMAL)
        # self.text_box.insert(END, pr1)
        # self.text_box.configure(state=DISABLED)
        # self.text_box.see(END)
    ...
    def nlp(self,prompt):
        self.response = ai.processCommand(prompt)
    
    def mic_message_insert(self):
